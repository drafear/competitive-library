...
ll inv(ll n);
ll power(ll x, ll n) { if (n < 0) return inv(power(x, -n)); ll res = 1; for (ll i = 1; i <= n; i <<= 1) { if (i & n) res = mul(res, x); x = mul(x, x); } return res; }
...
vector<ll> fact;
void init_fact(ll n) { fact.assign(n+1, 1); rep(i, 1, fact.size()) fact[i] = mul(fact[i-1], i); }
